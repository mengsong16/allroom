...
class SingleEnvExperiment(Experiment):
    ...
    def train(self, frames=np.inf, episodes=np.inf):
        self.her = self._preset.hyperparameters.get('her', False)
        print("======> Use HER: %s"%self.her)
    ...
    def _run_training_episode(self):
        ...
        # relabel trajectory and add extra data to the replay buffer
        if self.her:
            self._agent.replay_buffer.relabel(
    ...
    def _run_test_episode(self, test_agent):
        ...
        state = self._env.reset()
        # print("------- test reset --------------")
        # print(self._env.device)
        # print(state.device)
        # print(state['observation'].device)
        # #exit()
        # print("--- before test_agent.act-----")
